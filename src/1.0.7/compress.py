...
with open(enwik_output, "w+b") as fo:
    with open(enwik, "r", encoding="utf-8") as f:
        while True:
            ...
            for word in words:
                if word in final_dict_of_all_nodes:
                    nodes_dict_words_in_this_line[word] = final_dict_of_all_nodes[word]
            ...
            while iter_index < len(c):
                ...
                if root is None:
                    ...
                    if current_node.character in pointer.final_children_dict:
                        encoded_contents = encoded_contents + pointer.final_children_dict[current_node.character].encoded_string
                        pointer = final_dict_of_all_nodes[pointer.final_children_dict[current_node.character].node_character]
                    else:
